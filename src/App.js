...
import { Moon, Sun, Mail, Phone, Linkedin, Github, Menu, X, ChevronDown, Download, CloudDownload } from 'lucide-react';
...
const portfolioData = {
  ...
  projects: [
    ...
    {
      name: "T-Chat",
      description: "Task management and team collaboration app with shared tasks, calendar view, and performance analytics",
      image: chat1,
    ...
    {
      name: "Space",
      description: "Medical library app for medical students with lecture summaries, study materials, and order management",
      image: sp1,
    ...
    {
      name: "Ninja",
      description: "Delivery and ERP system with order management, real-time tracking, attendance, and employee management",
      image: ninja1,
    ...
    {
      name: "Request",
      description: "Contractors and project management app with budget tracking, team collaboration, and performance analytics",
      image: req1,
    ...
    {
      name: "Wasfa",
      description: "Recipe and cooking app with meal categories, ingredients lists, and beautiful food photography",
      image: w4,
      images: [w3, w2, w1, w4],
    ...
    {
      name: "Chess Game",
      description: "Interactive chess game with checkmate detection, move validation, and clean game interface",
       image: chess1,
    ...
    {
      name: "Quran Time",
      description: "Quran reading app with Surah navigation, recitation timing, favorites, and beautiful Arabic typography",
      image: q1,
      images: [q1,q2],
  ...
  skills: {
    technical: ["Dart", "Flutter", "Bloc Pattern", "Clean Architecture", "Git & GitHub", "RESTful APIs", "Firebase", "Supabase", "Node.js", "Express.js", "MongoDB", "SQL", "Python", "JavaScript", "GraphQL"],
    concepts: ["Data Structures", "OOP", "SOLID Principles", "Design Patterns", "Clean Code", "CI/CD", "Testing"],
    soft: ["Agile Development", "Communication Skills", "Team Player", "Problem-Solving"]
  }
};
...
const Portfolio = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? (darkMode ? 'bg-gray-900/95 backdrop-blur-sm shadow-lg' : 'bg-white/95 backdrop-blur-sm shadow-lg') : ''}`}>
            ...
            <div className="hidden md:flex space-x-8">
              {['Home', 'About', 'Experience', 'Projects', 'Skills'].map((item) => (
                <button
                  ...
                  className={`transition-colors ${activeSection === item.toLowerCase() ? 'text-pink-500' : darkMode ? 'text-gray-300 hover:text-pink-400' : 'text-gray-700 hover:text-pink-500'}`}
                >
                  {item}
                </button>
              ))}
            ...
            <div className="px-4 py-4 space-y-3">
              {['Home', 'About', 'Experience', 'Projects', 'Skills', 'Contact'].map((item) => (
                <button
                  key={item}
                  onClick={() => scrollToSection(item.toLowerCase())}
                  className={`block w-full text-left px-4 py-2 rounded-lg transition-colors ${activeSection === item.toLowerCase() ? 'bg-pink-500/20 text-pink-500' : darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}
                >
                  {item}
                </button>
              ))}
      ...
      <section id="home" className="min-h-screen flex items-center justify-center px-4 pt-16">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-bold mb-4 animate-slide-up">
            Hi, I'm <span className="bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">Adel Mostafa</span>
          </h1>
          <p className="text-2xl md:text-3xl mb-8 animate-slide-up" style={{animationDelay: '0.1s'}}>
            {portfolioData.personal.title}
          </p>
          <p className="text-lg md:text-xl text-gray-400 max-w-3xl mx-auto mb-12 animate-slide-up" style={{animationDelay: '0.2s'}}>
            Flutter Developer | Mobile App Specialist 
          </p>
          <div className="flex flex-wrap justify-center gap-4 animate-slide-up" style={{animationDelay: '0.3s'}}>
            <a href={portfolioData.personal.linkedin} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-gradient-to-r from-pink-500 to-purple-600 rounded-lg hover:shadow-lg hover:scale-105 transition-all">
              <Linkedin className="inline mr-2" size={20} />
              LinkedIn
            </a>
            <a href={portfolioData.personal.github} target="_blank" rel="noopener noreferrer" className={`px-6 py-3 rounded-lg border-2 ${darkMode ? 'border-gray-700 hover:border-pink-500' : 'border-gray-300 hover:border-pink-500'} hover:shadow-lg hover:scale-105 transition-all`}>
              <Github className="inline mr-2" size={20} />
              GitHub
            </a>
            <a href={resumePDF} target="_blank" rel="noopener noreferrer" className={`px-6 py-3 rounded-lg border-2 ${darkMode ? 'border-gray-700 hover:border-pink-500' : 'border-gray-300 hover:border-pink-500'} hover:shadow-lg hover:scale-105 transition-all`}>
      ...
      <section id="about" className={`py-20 px-4 ${darkMode ? 'bg-gray-800/50' : 'bg-white'}`}>
        <div className="max-w-6xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-bold mb-12 text-center">
            About <span className="bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">Me</span>
          </h2>
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-3">
                  ...
                  <a href={`mailto:${portfolioData.personal.email}`} className="hover:text-pink-500 transition-colors">
                    {portfolioData.personal.email}
                  </a>
                </div>
                <div className="flex items-center space-x-3">
                  <Phone className="text-pink-500" size={20} />
                  <a href={`tel:${portfolioData.personal.phone}`} className="hover:text-pink-500 transition-colors">
            ...
            <div className={`p-8 rounded-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
              ...
              <div className="space-y-2">
                <p className="text-xl font-semibold text-pink-500">Faculty of Computing and Data Science</p>
                <p className={darkMode ? 'text-gray-400' : 'text-gray-600'}>Alexandria University</p>
                <p className={darkMode ? 'text-gray-400' : 'text-gray-600'}>2021 - 2025</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="experience" className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-bold mb-12 text-center">
            Work <span className="bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">Experience</span>
          </h2>
          <div className="space-y-8">
            {portfolioData.experience.map((exp, index) => (
              <div key={index} className={`p-8 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white'} hover:shadow-xl transition-all hover:scale-[1.02]`}>
                <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4">
                  <div>
                    <h3 className="text-2xl font-bold text-pink-500">{exp.role}</h3>
                    <p className="text-xl font-semibold mt-1">{exp.company}</p>
                    <p className={`mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{exp.location}</p>
                  </div>
                  <span className={`mt-2 md:mt-0 px-4 py-2 rounded-full text-sm ${darkMode ? 'bg-pink-500/20 text-pink-400' : 'bg-pink-100 text-pink-600'}`}>
                    {exp.period}
                  </span>
                </div>
                <ul className="space-y-2 mt-4">
                  {exp.highlights.map((highlight, i) => (
                    <li key={i} className={`flex items-start space-x-3 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
      ...
      <section id="projects" className={`py-20 px-4 ${darkMode ? 'bg-gray-800/50' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-bold mb-12 text-center">
            Featured <span className="bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">Projects</span>
          </h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {portfolioData.projects.map((project, index) => (
              <div key={index} className={`rounded-2xl overflow-hidden ${darkMode ? 'bg-gray-900' : 'bg-gray-50'} hover:shadow-2xl transition-all hover:scale-105 group`}>
                <div
                ...
                >
                  <div className="text-6xl"><img src={project.image} alt={project.name} className="w-full h-full object-cover"/></div>
                </div>
                <div className="p-6">
                  <h3 className="text-xl font-bold mb-2 group-hover:text-pink-500 transition-colors">{project.name}</h3>
                  <p className={`mb-4 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{project.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((tech, i) => (
                      <span key={i} className={`px-3 py-1 rounded-full text-xs ${darkMode ? 'bg-pink-500/20 text-pink-400' : 'bg-pink-100 text-pink-600'}`}>
                        {tech}
                      </span>
                    ))}
                  </div>
      ...
      <section id="skills" className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-bold mb-12 text-center">
            Technical <span className="bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">Skills</span>
          </h2>
          <div className="grid md:grid-cols-3 gap-8">
            <div className={`p-8 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <h3 className="text-2xl font-bold mb-6 text-pink-500">Technical</h3>
              <div className="flex flex-wrap gap-2">
                {portfolioData.skills.technical.map((skill, i) => (
                  <span key={i} className={`px-4 py-2 rounded-lg ${darkMode ? 'bg-gray-900' : 'bg-gray-100'} hover:bg-gradient-to-r hover:from-pink-500 hover:to-purple-600 hover:text-white transition-all cursor-default`}>
                    {skill}
                  </span>
                ))}
              </div>
            ...
            <div className={`p-8 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <h3 className="text-2xl font-bold mb-6 text-purple-500">Concepts</h3>
              <div className="flex flex-wrap gap-2">
            ...
            <div className={`p-8 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <h3 className="text-2xl font-bold mb-6 text-blue-500">Soft Skills</h3>
              <div className="flex flex-wrap gap-2">
      ...
      <section id="contact" className={`py-20 px-4 ${darkMode ? 'bg-gray-800/50' : 'bg-white'}`}>
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-4xl md:text-5xl font-bold mb-8">
            Let's <span className="bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">Connect</span>
          </h2>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            ...
            <a href={portfolioData.personal.linkedin} target="_blank" rel="noopener noreferrer" className={`flex items-center space-x-2 px-8 py-4 rounded-lg border-2 ${darkMode ? 'border-gray-700 hover:border-pink-500' : 'border-gray-300 hover:border-pink-500'} hover:shadow-lg hover:scale-105 transition-all`}>
              <Linkedin size={24} />
              <span>LinkedIn</span>
            </a>
            <a href={portfolioData.personal.github} target="_blank" rel="noopener noreferrer" className={`flex items-center space-x-2 px-8 py-4 rounded-lg border-2 ${darkMode ? 'border-gray-700 hover:border-pink-500' : 'border-gray-300 hover:border-pink-500'} hover:shadow-lg hover:scale-105 transition-all`}>
              <Github size={24} />
              <span>GitHub</span>
            </a>
          </div>
      ...
      {selectedProject && (
        ...
        >
          <div
            className={`relative max-w-5xl w-full max-h-[90vh] ${darkMode ? 'bg-gray-900' : 'bg-white'} rounded-2xl overflow-hidden shadow-2xl`}
          ...
          >
            ...
            <div className="p-6 border-b border-gray-700">
              <h3 className="text-2xl font-bold text-pink-500 mb-2">{selectedProject.name}</h3>
